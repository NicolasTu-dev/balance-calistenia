...
export default async function ProgramaDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-12">
      ...
      <h1 className="mt-2 text-4xl font-semibold tracking-tight">{program.name}</h1>

      <div className="mt-6 rounded-2xl border border-white/10 bg-white/5 p-6">
        <h2 className="text-lg font-semibold">Descripción</h2>
        <p className="mt-3 text-sm opacity-80">
          {program.description?.trim() || getLevelBlurb(program.level) || "Sin descripción."}
        </p>
      </div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-5 md:grid-cols-2">
        <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
          ...
          <p className="mt-2 text-sm opacity-80">
            (v2) Si querés, sumamos campos en DB como <code>audience</code> / <code>requirements</code>.
          </p>
        </div>
        <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
